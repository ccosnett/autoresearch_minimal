...
3. **Read the in-scope files**: The repo is small. Read these files for full context:
   ...
   - `prepare.py` — fixed constants, data generation, and evaluation. Do not modify.
   - `train.py` — the file you modify. Training algorithm, hyperparameters, feature engineering.
...
Each experiment runs locally. The training script runs for a **fixed time budget** (defined in `prepare.py`). You launch it simply as: `python train.py`.

**What you CAN do:**
- Modify `train.py` — this is the only file you edit. Everything is fair game: algorithm, optimizer, hyperparameters, feature engineering, closed-form solutions, regularization, etc.

**What you CANNOT do:**
- Modify `prepare.py`. It is read-only. It contains the fixed evaluation, data generation, and training constants (time budget, etc).
- Install new packages or add dependencies. You can only use what's already in `pyproject.toml`.
- Modify the evaluation harness. The `evaluate_mse` function in `prepare.py` is the ground truth metric.
